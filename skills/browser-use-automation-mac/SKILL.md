...
- Use one page-control implementation only: a `mac_computer_use_tool` cell with `plane="bu"` whose code imports `seed_browser_use as bu`.
...
- Keep page interaction inside `mac_computer_use_tool` with `plane="bu"`. Bash, HTTP clients, and shell scripts are not alternate browser-control paths.
...
```python
mac_computer_use_tool(plane="bu", code="...", title="Inspect the pricing page")
```
